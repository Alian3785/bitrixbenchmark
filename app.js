...
function loadState() {
  try {
    ...
    const completedLevels = Array.isArray(merged.completedLevels) ? merged.completedLevels : [];
    merged.completedLevels = [...new Set(completedLevels.map(Number))].filter(
      (level) => Number.isInteger(level) && level >= 0 && level < lessonNames.length
    );
    return merged;
  } catch {
    return { ...initialState };
...
function markCurrentLessonComplete() {
  if (!state.completedLevels.includes(currentLevel)) {
    state.completedLevels = [...state.completedLevels, currentLevel];
    saveState();
  }
  renderPath();
}
...
function playApplause() {
  ...
  for (let index = 0; index < 22; index += 1) {
    ...
    const start = now + 0.03 + index * 0.045 + Math.random() * 0.025;
    const bufferSize = Math.max(1, Math.floor(ctx.sampleRate * duration));
    const buffer = ctx.createBuffer(1, bufferSize, ctx.sampleRate);
    const data = buffer.getChannelData(0);
    for (let sampleIndex = 0; sampleIndex < bufferSize; sampleIndex += 1) {
      data[sampleIndex] = (Math.random() * 2 - 1) * (1 - sampleIndex / bufferSize);
    }
  ...
  [523.25, 659.25, 783.99, 1046.5, 1318.51].forEach((frequency, index) => {
    playTone(ctx, now + index * 0.09, frequency, 0.2, "triangle", 0.08);
  });
}
...
function celebrateLessonComplete() {
  playApplause();
  setMascot("good", true);
  ...
  banner.textContent = "Урок пройден";
  layer.append(banner);

  const colors = ["#47c653", "#2176ff", "#ffc947", "#ff6d4d", "#7c3aed", "#14b8a6"];
  for (let index = 0; index < 90; index += 1) {
    const piece = document.createElement("span");
    piece.className = index % 5 === 0 ? "tinsel-streamer" : "confetti-piece";
    piece.style.left = `${Math.random() * 100}%`;
    piece.style.setProperty("--fall-x", `${Math.random() * 220 - 110}px`);
    piece.style.setProperty("--spin", `${Math.random() * 720 - 360}deg`);
    piece.style.setProperty("--color", colors[index % colors.length]);
    piece.style.animationDelay = `${Math.random() * 0.35}s`;
    piece.style.animationDuration = `${1.9 + Math.random() * 1.35}s`;
    layer.append(piece);
  }
...
function renderPath() {
  els.pathNodes.forEach((node) => {
    ...
    node.classList.toggle("is-complete", state.completedLevels.includes(level));
  });
  updateNextLevelButton();
}

function updateNextLevelButton() {
  const isComplete = state.completedLevels.includes(currentLevel);
  const hasNextLevel = currentLevel < lessonNames.length - 1;
  const canGoNext = isComplete && hasNextLevel;

  els.nextLevel.disabled = !canGoNext;
  els.nextLevel.setAttribute("aria-disabled", String(!canGoNext));
  els.nextLevel.textContent = hasNextLevel ? "Следующий уровень" : "Все уровни пройдены";
  els.nextLevel.setAttribute(
    "aria-label",
    canGoNext
      ? `Перейти к уроку ${currentLevel + 2}: ${lessonNames[currentLevel + 1]}`
      : hasNextLevel
        ? "Следующий уровень станет доступен после прохождения урока"
        : "Все уровни пройдены"
  );
}
...
function checkQuizAnswer(button, good) {
  const completesLesson = good && currentWord === wordsForLevel().length - 1;
  ...
  if (good) setTimeout(() => nextWord({ celebrateOnWrap: completesLesson }), 700);
}

function nextWord(options = {}) {
  const levelWords = wordsForLevel();
  const wasLastWord = currentWord === levelWords.length - 1;
  if (wasLastWord) markCurrentLessonComplete();
  currentWord = (currentWord + 1) % levelWords.length;
  renderWord();
  if (options.celebrateOnWrap && wasLastWord) {
    celebrateLessonComplete();
  }
}

function goToLesson(level) {
  currentLevel = Math.max(0, Math.min(level, lessonNames.length - 1));
...
function bindEvents() {
  ...
  els.pathNodes.forEach((node) => {
    node.addEventListener("click", () => {
      goToLesson(Number(node.dataset.level));
    });
  });
  els.nextLevel.addEventListener("click", () => {
    if (els.nextLevel.disabled) return;
    goToLesson(currentLevel + 1);
  });
